...
// std::vector<PACKET> pf_buffer(buffer_size);
// std::array<uint64_t, 16> pf_buffer;
//naman
...
//	mshr_full = 0;
    ...
    void add_nonfifo_queue(PACKET_QUEUE *queue, PACKET *packet), //@Vishal: Updated from add_mshr
		...
		stlb_replacement_final_stats(),
         ...
         l1d_prefetcher_operate(uint64_t addr, uint64_t ip, uint8_t cache_hit, uint8_t type, uint8_t critical_ip_flag, uint64_t full_physical_address), //, uint64_t prefetch_id),
